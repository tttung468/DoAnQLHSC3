...
{	
	...
	protected void applyUI() 
	{
		...
		if(index != null)
		{
			titleLabel.setText("SỬA/XÓA GIÁO VỤ");
			codeTextField.setEditable(false);
		}
		else
			titleLabel.setText("THÊM GIÁO VỤ");
	}
	@Override
	...
	public boolean delete()
	{
		...
		officeStaffService.delete(deletedIds);
	...
	public boolean update()
	{
		if(avatarFile != null)
		{
			String destFolder = getClass().getResource(SystemConstant.OFFICESTAFF_IMAGES_PATH).toString().substring(6);
			String fileName = UploadUtils.uploadImage(avatarFile, destFolder);
			model.setAvatarpath(SystemConstant.OFFICESTAFF_IMAGES_PATH + "/" + fileName);
		}
		
		model = officeStaffService.updateOne(model, accountModel);
	...
	{	
		Map<String, String> map = officeStaffService.validate(model);
		...
		if(map.get("alert").equals("success"))
		{
			...
			model = officeStaffService.insertOne(model, accountModel);
			messageLabel.setText(model.getMessage());
